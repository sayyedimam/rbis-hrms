...
class LeaveRepository:
    ...
    # Holiday operations
    def create_holiday(self, holiday_data: dict) -> Holiday:
        """Create new holiday"""
        holiday = Holiday(**holiday_data)
        self.db.add(holiday)
        return holiday

    def get_holiday_by_id(self, holiday_id: int) -> Optional[Holiday]:
        """Get holiday by ID"""
        return self.db.query(Holiday).filter(Holiday.id == holiday_id).first()

    def update_holiday(self, holiday: Holiday, holiday_data: dict) -> Holiday:
